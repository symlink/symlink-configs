;;; psgml.el --- XML/SGML-editing mode with parsing support
...
;; Major mode for editing the XML/SGML document-markup language.
...
"*A list of XML/SGML entity catalogs to be searched first when parsing the buffer.
...
The files are in the format defined in the XML/SGML Open Draft Technical
...
;;; sgmls is a free XML/SGML parser available from
;;; ftp.uu.net:pub/text-processing/sgml
;;; Its error messages can be parsed by next-error.
;;; The -s option suppresses output.

(defvar sgml-validate-command   "nsgmls -s %s %s"
  "*The shell command to validate an XML/SGML document.
...
%s means the XML/SGML declaration specified in the sgml-declaration variable
...
(defvar sgml-xml-validate-command 
  "SP_CHARSET_FIXED=YES SP_ENCODING=XML nsgmls -wxml -mdeclaration/xml.soc -gues %s %s"
...
(defvar sgml-declaration nil
  "*If non-nil, this is the name of the XML/SGML declaration file.")
(put 'sgml-declaration 'sgml-type 'file-or-nil)

(defvar sgml-xml-declaration nil
  "*If non-nil, this is the name of the XML/SGML declaration for XML files.")
...
(defvar sgml-mode-map nil "Keymap for XML/SGML mode")
...

;;;; XML/SGML mode: template functions
  ...
  (let ((last-values (assoc menuname sgml-last-options-menu-values)))
    (when (or (null last-values)
              (sgml-any-option-changed (cdr last-values)
                                       option-vars))
      (condition-case err
          (easy-menu-change '("XML/SGML") menuname
...

;;;; XML/SGML mode: major mode definition

;;; This section is mostly from sgml-mode by James Clark.

;;;###autoload
(defun sgml-mode ()
  "Major mode for editing XML/SGML.\\<sgml-mode-map>
Makes > display the matching <.  Makes / display matching /.
Use \\[sgml-validate] to validate your document with an XML/SGML parser.
...
sgml-omittag  Set this to reflect OMITTAG in the XML/SGML declaration.
sgml-shorttag  Set this to reflect SHORTTAG in the XML/SGML declaration.
sgml-namecase-general  Set this to reflect NAMECASE GENERAL in the XML/SGML declaration.
...
"
  ...
  ;; This is desirable because XML/SGML discards a newline that appears
...
(defvar sgml-mode-markup-syntax-table nil
  "Syntax table used for scanning XML/SGML markup.")
...
--\\([^-]\\|-[^-]\\)*--\\)*\\["
	    ...
	    ;; declaration subset); start tag; end tag; XML/SGML declaration.
	    (if blinkpos
		()
	      (goto-char oldpos)
	      (condition-case ()
...
(defun sgml-validate (command)
  "Validate an XML/SGML document.
Runs COMMAND, a shell command, in a separate process asynchronously
with output going to the buffer *compilation*.
You can then use the command \\[next-error] to find the next error message
and move to the line in the XML/SGML document that caused it."
  (interactive
  ...
  (compile-internal command "No more errors" "XML/SGML validation"
		    nil
