...
;; $Revision: 1.54 $ $Date: 2002/12/14 18:37:36 $ 
...
variable ANT_HOME."
  ...
  (let* ((ANT_HOME_ENV (getenv "ANT_HOME"))
         (ant-home (if (string= "" jde-ant-home)
...
(defun jde-ant-build(buildfile target &optional interactive-args)
  ...
  (interactive
   (let (buildfile history target interactive-args)
...
(defun jde-ant-add-to-history (buildfile history)
    ...
    (if found
      ...
      (setq jde-ant-interactive-target-history 
            (append 
             ...
             (list (list buildfile (car history))))))))
