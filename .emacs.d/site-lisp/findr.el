...
;;(let ((result nil))
;;  (dolist (el (list ".backups" "_darcs" "CVS" ".svn"))
...
(defcustom findr-skip-directory-regexp "^\\.backups$\\|^_darcs$\\|^CVS$\\|^\\.svn$"
...
with the command \\[tags-loop-continue]."
  (interactive (let ((search-for (findr-read-search-regexp "Replace text through files (regexp): ")))
                 (list search-for
                       (read-from-minibuffer (format "Replace '%s' by: " search-for)
